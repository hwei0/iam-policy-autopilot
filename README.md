...
To enable the IAM Policy Autopilot Kiro Power, first **complete the installation steps** specified in this README file. Then, within Kiro:
1. Go to the "Powers" menu in the menubar on the left-hand-side.
2. Click `Add Custom Power` -> `Import power from Github`
3. In the text prompt that then appears, enter the link to this github repository, and press enter: `https://github.com/awslabs/iam-policy-autopilot`

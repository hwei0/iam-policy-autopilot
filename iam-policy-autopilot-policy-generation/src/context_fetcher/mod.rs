...
use crate::{
    context_fetcher::terraform_state::{TerraformShowReader, TerraformStateContext},
...
impl TerraformProjectExplorer {
    pub(crate) fn new(terraform_dir: &PathBuf) -> Result<Self, ExtractorError> {
        ...
        Ok(TerraformProjectExplorer {
            terraform_state_context: TerraformStateContext::read_from_terraform_reader(
                terraform_show_reader,
            )?,
        })
    }
}

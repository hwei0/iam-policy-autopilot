...
impl TerraformStateContext {
    ...
    pub(crate) fn read_from_terraform_reader(
        ...
        let map = terraform_show.terraform_output.as_object().ok_or(
            ExtractorError::terraform_state_parse(
                "Terraform show object is not a map".to_string(),
                JsonValue::to_string(&terraform_show.terraform_output),
            ),
        )?;

        let values_map = map
            .get("values")
            .ok_or(ExtractorError::terraform_state_parse(
        ...
        let root_module_map =
            values_map
                .get("root_module")
                .ok_or(ExtractorError::terraform_state_parse(
                    "Terraform show object does not have values.root_module field".to_string(),
                    JsonValue::to_string(&terraform_show.terraform_output),
                ))?;
        ...
        let resources =
            root_module_map
                .get("resources")
                .ok_or(ExtractorError::terraform_state_parse(
                    "Terraform show object does not have values.root_module.resources field"
                        .to_string(),
                    JsonValue::to_string(&terraform_show.terraform_output),
                ))?;

        let resource_arr = resources
            .as_array()
            .ok_or(ExtractorError::terraform_state_parse(
                "Terraform resources object is not an array".to_string(),
                JsonValue::to_string(&terraform_show.terraform_output),
            ))?;
...
impl TerraformShowReader {
    pub(crate) fn retrieve_terraform_state(
        ...
        if !output.status.success() {
            Err(ExtractorError::terraform_state_command(
                cmd_str,
                String::from_utf8_lossy(&output.stderr).to_string(),
            ))
        } else {
            let json = String::from_utf8_lossy(&output.stdout).to_string();
            Ok(TerraformShowReader {
                terraform_output: from_str(&json)?,
            })
        }
    }
}
